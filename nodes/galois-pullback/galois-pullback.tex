...
\begin{definition}
  For $G$ a topological group, we denote by $\Sub_G$ the set of open normal subgroups of $G$, which is a poset under inclusion.
\end{definition}
...
\begin{proposition}
  ...
  \begin{enumerate}
  \item \label{preimage-injective} the preimage map $\Sub_G \to \Sub_A$, defined by $N \mapsto \phi^{-1}(N)$, is injective;
  \item \label{join-preserved} $\phi^{-1}(N_1N_2) = \phi^{-1}(N_1)\phi^{-1}(N_2)$ for
    $N_1,N_2 \in \Sub_G$.
  \end{enumerate}
\end{proposition}
